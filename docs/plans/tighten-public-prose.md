...
**Status:** all stages complete (20/06/2026) — all four narrative docs tightened to the house
voice; em-dashes cut in flowing prose (methodology.md 100→26, remainder are list/table dashes),
meaning preserved, `mkdocs build --strict` and the consistency hook clean. Pending review + merge.
...
## Stage 2 — methodology.md, front half (Operating principle → §6)   [done]
- [x] Section pass (heaviest §1); heading em-dashes case-by-case (no `#anchor` targets them).
- Acceptance: voice-only diff; §-refs and the §1 table intact; build --strict clean.

## Stage 3 — methodology.md, back half (§7 → §11)   [done]
- [x] Section pass (heaviest §8, §10, §11).
- Acceptance: as Stage 2; full-file build --strict clean.

## Stage 4 — index.md + worked-example.md   [done]
- [x] Light pass; leave fenced code, GitHub blob URLs, and the does/not/exist.md demo untouched.
