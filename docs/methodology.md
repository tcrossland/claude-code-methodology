...
- **Avoid `bypassPermissions` as a habit.** It skips prompts and will happily write to protected paths (`.git`, `.claude`, and so on). The disciplined position is `acceptEdits` on a feature branch or in a worktree: you get flow without the foot-gun, and a bad run is one `git restore` (or one checkpoint rewind) away.
...
- A **`git` `PreToolUse` guard** (Appendix C) that blocks `git add -A` / `git add .`, `git commit`, and `git push` unless you approve — the §6 hook pattern pointed at the accidental-commit class. Pair it with `.gitignore` discipline and the security-review plugin for credential leaks.
