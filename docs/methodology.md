...
Hooks can also be scoped *inside* a subagent's frontmatter (they fire only while that agent runs and are cleaned up afterwards) — the clean way to make `test-writer` physically unable to write outside `Tests/`, regardless of what it is asked. Use the `$CLAUDE_PROJECT_DIR` prefix for hook script paths so they resolve regardless of working directory. See Appendix C.
...
---
name: add-importer
description: Scaffold a new bank-statement importer following project conventions. Use when adding support for a new bank's CSV or PDF export.
allowed-tools: Read, Glob, Grep, Edit, Write, Bash
---

# Add a bank importer

When asked to add an importer for a new bank:

1. Read an existing importer as the template (e.g. `Importers/StarlingCSV.swift`).
2. Confirm the source format against a real sample before writing any parsing
   code — never guess column order or date locale.
3. Implement the parser conforming to the `StatementImporter` protocol.
4. Route every transaction through the double-entry write layer; never write a
   balance directly (see the Invariants in CLAUDE.md).
5. Add tests: happy path, empty file, malformed row.
6. Run the suite and report results.

Out of scope: changing the `StatementImporter` protocol itself — raise that as a
separate decision (ADR).
```
